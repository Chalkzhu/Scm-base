...
// 更多筛选
const MoreFilter = ({ filters, setFilterValue }) => {
  const [checkedValues, setCheckedValues] = useState(() => filters.filter(v => v.fixed).map(v => v.field));

  // 排序后的数组
  const options = useMemo(() => {
    return filters.sort((a, b) => !!b.fixed - !!a.fixed)
  }, [filters]);

  // 是否可置顶, 第一项无置顶操作, 所以这里的index > 0
  const isTop = useCallback((item) => {
    return checkedValues.indexOf(item.field) > 0;
  }, [checkedValues]);
  ...
  // 置顶操作, 防止频繁操作, 建议加上节流
  const handleTop = (e, item) => { e.stopPropagation(); setFilterValue(item, 'top'); };
    ...
    <div className="filter_dropdown">
      <div className="filter_header">
        <div className="filter_header_operate">
          ...
          <div><span className='filter_reset' onClick={handleReset}>重置</span></div>
        </div>
      </div>
      <Checkbox.Group value={checkedValues} className="filter_body">
        <VirList options={options}>
          {
            ({ item, ...resetProps }) => <div {...resetProps} className="filter_item" onClick={(e) => onChange(e, item)}>
              <Checkbox value={item.field} className="filter_item-content">{item.title}</Checkbox>
              <div className="filter_item_operate">
                {isTop(item) && <IconFont type="lmweb-vertical-align-top" className="icon_top" onClick={(e) => handleTop(e, item)} />}
              </div>
            </div>
          }
        </VirList>
